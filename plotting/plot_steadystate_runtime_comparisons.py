...
for s, states in enumerate(states_set):
    for n, noise in enumerate(noise_values):
        for e, er in enumerate(evidence_rates):
            ...
            # Complete graph
            if connectivity_value == 1.0:
                plt.savefig("../../results/graphs/pddm-network/runtime_{}_states_{:.2f}_er_{:.2f}_noise.pdf".format(states, er, noise))
            # Evidence-only graph
            elif connectivity_value == 0.0:
                plt.savefig("../../results/graphs/pddm-network/runtime_ev_only_{}_states_{:.2f}_er_{:.2f}_noise.pdf".format(states, er, noise))
            plt.clf()