    ...
    for n, noise in enumerate(noise_levels):
        for a, agents in enumerate(agents_set):
            ...
            for _ in range(2):
                ...
                # Complete graph
                if conn == 1.0:
                    plt.savefig("../../results/graphs/pddm-network/uncertainty_trajectory_{}_agents_{}_states_{:.2f}_noise{}.pdf".format(agents, states, noise, closure))
                # Evidence-only graph
                elif conn == 0.0:
                    plt.savefig("../../results/graphs/pddm-network/uncertainty_trajectory_ev_only_{}_agents_{}_states_{:.2f}_noise{}.pdf".format(agents, states, noise, closure))
                plt.clf()