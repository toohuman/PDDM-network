...
for s, states in enumerate(states_set):
    for n, noise in enumerate(noise_values):
        for a, agents in enumerate(agents_set):
            ...
            results = np.array([[0.0 for x in evidence_rates] for y in closure])
            lowers = np.array([[0.0 for x in evidence_rates] for y in closure])
            uppers = np.array([[0.0 for x in evidence_rates] for y in closure])
            ...
            for c, cl in enumerate(closure):
                for e, er in enumerate(evidence_rates):
                    ...
                    lowers[c][e] = data[PERC_LOWER - 1]
                    uppers[c][e] = data[PERC_UPPER - 1]
                    results[c][e] = np.average(data)
            ...
            for c, cl in enumerate(closure):
                ax = sns.lineplot(evidence_rates, results[c], linewidth = 2, color=sns.color_palette()[c], label=closure_strings[c])
                plt.fill_between(evidence_rates, lowers[c], uppers[c], facecolor=sns.color_palette()[c], edgecolor="none", alpha=0.3, antialiased=True)
            plt.axhline(expected_error(noise, states), color="red", linestyle="dotted", linewidth = 2)
            plt.xlabel(r"Evidence rate $r$")
            ...
            # Complete graph
            if connectivity_value == 1.0:
                plt.savefig("../../results/graphs/pddm-network/error_comps_{}_states_{}_agents_{:.2f}_noise.pdf".format(states, agents, noise))
            # Evidence-only graph
            elif connectivity_value == 0.0:
                plt.savefig("../../results/graphs/pddm-network/error_comps_ev_only_{}_states_{}_agents_{:.2f}_noise.pdf".format(states, agents, noise))
            plt.clf()