        ...
        for c, cl in enumerate(closure):
            ...
            for e, er in enumerate(reversed(evidence_rates)):
                for a, agents in enumerate(agents_set):
                    ...
                    # import time
                    # time.sleep(2)
                    heatmap_results[e][a] = np.average(data)
            ...
            ax.set(xlabel='Agents', ylabel='Evidence rate')
            plt.tight_layout()
            plt.savefig("../../results/graphs/pddm-network/hm_error_comps_{}_states_{:.2f}_noise{}.pdf".format(states, noise, cl))
            plt.clf()