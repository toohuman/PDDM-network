...
for s, states in enumerate(states_set):
    for n, noise in enumerate(noise_values):
        ...
        print("Average Uncertainty: {} states | {:.2f} noise".format(states, noise))
        for e, er in enumerate(evidence_rates):
        ...
        # Complete graph
        if connectivity_value == 1.0:
            plt.savefig("../../results/graphs/pddm-network/uncertainty_complete_{}_states_{:.2f}_noise{}.pdf".format(states, noise, closure))
        # Evidence-only graph
        elif connectivity_value == 0.0:
            plt.savefig("../../results/graphs/pddm-network/uncertainty_complete_ev_only_{}_states_{:.2f}_noise{}.pdf".format(states, noise, closure))
        plt.clf()